...
if [[ "$DATABASE_HOSTNAME" && "$DATABASE_USERNAME" && "$DATABASE_PASSWORD" ]]; then
  ...
  sed -i "s/DATABASE_HOSTNAME/$DATABASE_HOSTNAME/g" ../etc/broker.xml
  sed -i "s/DATABASE_USERNAME/$RDS_USERNAME/g" ../etc/broker.xml
  sed -i "s/DATABASE_PASSWORD/$RDS_PASSWORD/g" ../etc/broker.xml
fi
